...
First, try to connect to the experience through the mcp.
I never complete this Car framework, i've got all the binders done but i get problems in creating other Cars, i were working in a class to do match Workspace.Car setting, if it have the same constraints and the same attachment properties i asume it would work, however when i try to run the car it doesn't run at all. How could we set up Workspace.Cart in order to work with the Binder? (Workspace.Car already does ).
Use the mcp connection and find out the needed configuration to properly match Workspace.Car, then finnish src/myNeverMoreS/car/src/Server/Controllers/CarInitializer.luau in order to make them work.
Note: src/myNeverMoreS/car/src/Server/Controllers/CarInitializer.luau needs to run in the command bar, if it's done in runtime the phisics constraints will cause some parts to fly away