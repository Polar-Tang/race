...
local JestGetType = require(script.Parent.Parent:WaitForChild('jest-get-type'))
local getType = JestGetType.getType
local isPrimitive = JestGetType.isPrimitive

local JestMatcherUtils = require(script.Parent.Parent:WaitForChild('jest-matcher-utils'))
...
local Utils = require(script.Parent:WaitForChild('utils'))-- ROBLOX deviation: skipped as Lua doesn't support ArrayBuffer
...
local function toBe(
	...
	if pass then
		...
		message = function()
			local deepEqualityName 			-- ROBLOX deviation: no map or set types for now so this check is always true so we omit it
